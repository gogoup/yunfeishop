define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'manystore/command/index',
                    add_url: 'manystore/command/add',
                    edit_url: '',
                    del_url: 'manystore/command/del',
                    multi_url: 'manystore/command/multi',
                    table: 'manystore_command',
                }
            });
        ...
        add: function () {
            ...
            $(document).on('change', "select[name='table']", function () {
                var that = this;
                Fast.api.ajax({
                    url: "manystore/command/get_field_list",
                    data: {table: $(that).val()},
                }, function (data, ret) {
            ...
            $(document).on('click', ".btn-command", function () {
                ...
                textarea.val('');
                Fast.api.ajax({
                    url: "manystore/command/command/action/command",
                    data: form.serialize(),
                }, function (data, ret) {
            ...
            $(document).on('click', ".btn-execute", function () {
                ...
                textarea.val('');
                Fast.api.ajax({
                    url: "manystore/command/command/action/execute",
                    data: form.serialize(),
                }, function (data, ret) {
