<?php /*a:1:{s:55:"F:\project\shop\ThinkAdmin\app\admin\view\api\upload.js";i:1648774486;}*/ ?>
define(['md5'], function (SparkMD5, allowMime) {
    ...
    return function (element, callable) {
        ...
        opt.uploader = layui.upload.render({
            url: '<?php echo sysuri("admin/api.upload/file"); ?>', auto: false, elem: element, accept: 'file', multiple: opt.mult,
            exts: opt.exts.join('|'), acceptMime: opt.mimes.join(','), choose: function (object) {
                ...
                opt.uploader.config.elem.next().val(''), layui.each(opt.files, function (index, file) {
                    ...
                    md5file(file).then(function (file) {
                        opt.elem.triggerHandler('upload.hash', file), jQuery.ajax("<?php echo sysuri('admin/api.upload/state'); ?>", {
    ...
    function md5file(file) {
        ...
        /*! 兼容不能计算文件 HASH 的情况 */
        var IsDate = '<?php echo htmlentities((isset($nameType) && ($nameType !== '')?$nameType:""),ENT_QUOTES); ?>'.indexOf('date') > -1;
