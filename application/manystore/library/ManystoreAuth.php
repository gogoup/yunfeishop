...
class ManystoreAuth
{
    ...
    public function getRuleList($uid)
    {
        ...
        if (2 == $this->config['auth_type'] && Session::has('_manystore_rule_list_' . $uid)) {
            return Session::get('_manystore_rule_list_' . $uid);
        }
        ...
        if (2 == $this->config['auth_type']) {
            //规则列表结果保存到session
            Session::set('_manystore_rule_list_' . $uid, $rulelist);
        }
