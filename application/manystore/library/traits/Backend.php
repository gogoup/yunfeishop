...
namespace app\manystore\library\traits;

use app\manystore\library\Auth;
use app\manystore\library\ManystoreAuth;
use Exception;
            ...
            if($this->shopIdAutoCondition){
                ...
                if ($has_shop_id) {
                    foreach ($insert as &$val) {
                        $val['shop_id'] = SHOP_ID ? SHOP_ID : 0;
                    }
                }
            }

            //是否包含store_id字段
            if($this->storeIdFieldAutoFill){
                $has_store_id = false;
                foreach ($fieldArr as $name => $key) {
                    if ($key == 'store_id') {
                        $has_store_id = true;
                ...
                if ($has_store_id) {
                    foreach ($insert as &$val) {
                        $val['store_id'] = STORE_ID ? STORE_ID : 0;
                    }
                }
            }
